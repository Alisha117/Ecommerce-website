      ...
      <DialogContent className="'max-w-5xl min-h-[90vh] max-h-[90vh] flex flex-col overflow-hidden bg-white">
        ...
        <div className="w-full h-full overflow-y-scroll border border-darkColor/20 rounded-md bg-white">
          <div className="">
                    ...
                    <div className="px-4 py-2 flex-grow">
                      <div className="flex justify-between items-start">
                        <Link
                          ...
                          onClick={() => setShowSearch(false)}
                        >
                          <h3 className="w-30 text-sm md:text-lg font-semibold text-gray-800 line-clamp-1">
                            {product.name}
                          </h3>
                          <p className=" w-30 text-sm text-gray-600 line-clamp-1">
                            {product?.intro}
                          </p>
                        </Link>
                        <PriceView
           ...
           ( <p> Nothing match with the keyword{" "} 
           <span className="underline text-red-600">{search}</span>.
            Please try something else. </p> 
          ) : ( 
          <p className="text-green-600 flex items-center justify-center gap-1"> 
          <Search className="w-5 h-5" /> Search and explore your products from Crystal. </p> 
        )}
         </div>
         )}
          </div>
